...
class RealInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # For pi0-FAST, we don't pad the state. For Real, we don't need to differentiate
        ...
        if "prev_actions" in data:
            # We are padding to the model prev_actions dim.
            # For pi0-FAST, this is a no-op (since action_dim = 7).
            prev_actions = transforms.pad_to_dim(data["prev_actions"], self.action_dim)
            inputs["prev_actions"] = prev_actions
...
class RealOutputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # For Real, we only return the first 7 actions (since the rest is padding).
